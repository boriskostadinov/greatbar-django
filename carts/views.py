...
def remove_cart(request, product_id, cart_item_id):
    ...
    try:
        if request.user.is_authenticated:
            cart_item = CartItem.objects.get(product=product, user=request.user, id=cart_item_id) # remove when logged in
        else:
...
def cart(request, total=0, quantity=0, cart_items=None):
    try:
        tax = 0
        grand_total = 0
        if request.user.is_authenticated:           # for logged users
